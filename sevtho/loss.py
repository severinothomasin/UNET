...
class DiceLoss(T_nn.Module):
    ...
    def forward(self, predicted, target):
        ...
        for index in range(batch):
            coefficient = self.__dice_coefficient__(
    ...
    def __dice_coefficient__(self, predicted, target):
        ...
        """
        smooth = 1
        product = T.mul(predicted, target)
...
class BCEDiceLoss(T_nn.Module):
    ...
    def forward(self, predicted, target):
        ...
        return T_nn_functional.binary_cross_entropy(predicted, target) + self.dice_loss(predicted, target)